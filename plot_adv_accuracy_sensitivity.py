...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot adv-ratio sensitivity using test accuracy at target epoch. "
            "Each figure is 1x4 attacks, with defenses as adv-accuracy lines."
        )
    )
    parser.add_argument(
        "--logs-root",
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/home/fengye/scratch/FL_Poison/plots_adv_sensitivity_epoch199"),
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.1,0.2,0.3",
        help="Comma-separated adv ratios on x-axis, e.g. 0.1,0.2,0.3.",
    )
    parser.add_argument(
        "--attacks",
        type=str,
...
def make_output_name(info: dict, epochs_tag: str, adv_tag: str):
    base = (
        f"adv_sensitivity_{info['iid']}_{info['dataset']}_{info['model']}_{info['algo']}"
        f"_alpha{info['alpha']}_clients{info['num_clients']}_lr{info['lr']}"
        f"_cfg{info['cfg']}_advset{adv_tag}_epochs{sanitize(epochs_tag.replace(',', '-'))}"
    )
...
def main():
    ...
    adv_values = parse_float_list(args.adv_values)
    if not adv_values:
        raise ValueError("--adv-values must contain at least one float.")
    adv_tokens = {format_num(v) for v in adv_values}
    adv_tag = "-".join(format_num(v) for v in adv_values)
    ...
    for path in all_txt:
        ...
        if args.iid_mode != "all" and meta["iid"] != args.iid_mode:
            ...
            continue
        if meta["adv"] not in adv_tokens:
            filtered_by_advset += 1
            continue
    ...
    for gk, records in grouped.items():
        ...
        by_attack_def_adv = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        baseline_by_adv = defaultdict(list)
        epochs_seen = set()
        for rec in records:
            epochs_seen.add(rec["epochs"])
            adv = float(rec["adv"])
            if rec["attack"] == BASELINE_ATTACK and rec["defense"] == BASELINE_DEFENSE:
                baseline_by_adv[adv].append(rec)
            if rec["attack"] in attacks:
                by_attack_def_adv[rec["attack"]][rec["defense"]][adv].append(rec)
        ...
        for ax, attack in zip(axes, attacks):
            ...
            ax.set_xlabel("Adversary Ratio", fontsize=BASE_FONT_SIZE * FONT_SCALE)
            ax.set_ylabel("Test Accuracy", fontsize=BASE_FONT_SIZE * FONT_SCALE)
            ax.grid(alpha=0.2)
            ax.tick_params(labelsize=BASE_FONT_SIZE * FONT_SCALE)

            plotted_on_ax = False
            defense_to_adv_stats = defaultdict(dict)
            for defense, adv_map in by_attack_def_adv.get(attack, {}).items():
                if defense in EXCLUDED_ATTACK_DEFENSES:
                    continue
                for adv, recs in adv_map.items():
                    ...
                    for seed, same_seed_recs in by_seed.items():
                        ...
                        seeds_used.add(seed)
                    if vals:
                        defense_to_adv_stats[defense][adv] = {
            ...
            for defense in sorted(defense_to_adv_stats.keys(), key=defense_sort_key):
                points = sorted(
                    [
                        (adv, defense_to_adv_stats[defense][adv])
                        for adv in adv_values
                        if adv in defense_to_adv_stats[defense]
                    ],
                    key=lambda kv: kv[0],
                )
                if not points:
            ...
            for adv in adv_values:
                ...
                if vals:
                    baseline_points.append(
                        (adv, float(np.mean(vals)), float(np.std(vals)), len(vals))
                    )
                    curve_ns.append(len(vals))
                    plotted_on_ax = True
                    plotted_any = True
            ...
            ax.set_xticks(adv_values)
            ax.set_xticklabels([format_num(v) for v in adv_values])
            if len(adv_values) >= 2:
                dx = (max(adv_values) - min(adv_values)) * 0.05
                ax.set_xlim(min(adv_values) - dx, max(adv_values) + dx)
        ...
        title = (
            f"{info['iid']} adv sensitivity | {info['dataset']}_{info['model']} | {info['algo']} | "
            f"alpha={info['alpha']} | target_epoch={args.target_epoch} | adv_set={adv_tag} | "
        ...
        out_name = make_output_name(info, epochs_tag, adv_tag)
    ...
    summary_rows.extend(
        [
            ...
            f"filtered_by_advset={filtered_by_advset}",
