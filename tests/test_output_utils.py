...
def test_epoch_metrics_writer_writes_train_only_rows(tmp_path):
    ...
    with EpochMetricsWriter(output_path, include_eval=False) as writer:
        writer.write_row(epoch=0, train_acc=0.5, train_loss=1.25)
        writer.write_row(epoch=1, train_acc=0.75, train_loss=0.5)

    with output_path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))

    assert rows == [
        {"epoch": "0", "train_acc": "0.5", "train_loss": "1.25"},
        {"epoch": "1", "train_acc": "0.75", "train_loss": "0.5"},
    ]
...
def test_parse_logs_reads_metrics_csv_prefer_eval_columns(tmp_path):
    ...
    with EpochMetricsWriter(output_path, include_eval=True) as writer:
        writer.write_row(epoch=0, train_acc=0.5, train_loss=1.0, eval_acc=0.25, eval_loss=1.5)
        writer.write_row(epoch=1, train_acc=0.8, train_loss=0.4, eval_acc=0.6, eval_loss=0.7)
    ...
    assert asrs == [None, None]
    assert asr_losses == [None, None]
