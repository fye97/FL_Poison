...
run_compute_canada_flpoison() {
  ...
  local experiment_id="${EXPERIMENT_ID_START:-${default_experiment_id}}"
  ...
  local total=$((n_scenarios * n_dist_specs * n_models * n_epochs * n_clients * n_lrs * n_advs * n_seeds * n_attacks * n_defenses))
  ...
  local result_root="${RESULT_ROOT:-${scratch_root}/FL_Poison/logs}"
  local log_dir=""
  local extra_tag=""
  local cfg_tag=""
  ...
  local exp_args=(--num_experiments "${num_experiments}" --experiment_id "${experiment_id}")
