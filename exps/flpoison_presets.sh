...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  echo "ERROR: source exps/flpoison_presets.sh from a wrapper script." >&2
  exit 2
fi

# 实验者主要在这里维护 attack/defense grid。
# `NUM_EXPERIMENTS` 表示每个配置需要跑多少个重复实验；
# 这些重复实验会被展开成独立 task，而不是在单个 task 里串行执行。
#
# 临时覆盖也可以直接传环境变量：
#   ATTACKS_CSV='NoAttack,MinMax'
#   DEFENSES_CSV='Mean,FLTrust'
#   NUM_EXPERIMENTS=3
#   EXPERIMENT_IDS_CSV='0,1,2'
