...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot num_clients sensitivity using test accuracy at target epoch. "
            "Each figure is 1x4 attacks, with defenses as client-count curves."
        )
    )
    parser.add_argument(
        "--logs-root",
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/home/fengye/scratch/FL_Poison/plots_num_clients_sensitivity_epoch199"),
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional comma-separated num_clients values on x-axis, e.g. 40,50,60. "
            "Empty means use all available values in each group."
        ),
    )
    parser.add_argument(
        "--attacks",
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=["all", "iid", "non-iid"],
        help="Filter by iid type.",
    )
    parser.add_argument(
        "--dataset-filter",
        type=str,
        default="",
        help="Optional comma-separated dataset filter, e.g. CIFAR100.",
    )
    parser.add_argument(
        "--adv-values",
        type=str,
        default="",
        help=(
            "Optional comma-separated adv ratios to keep, e.g. '0.1,0.2,0.3'. "
            "Empty means keep all adv values."
        ),
    )
    parser.add_argument(
        "--dpi",
...
def maybe_use_last_epoch_for_single_seed42(curve: dict, seed: str, seed_keys, status: str):
    if status != "short":
        return None, status
    if len(seed_keys) != 1 or seed != "42" or seed_keys[0] != "42":
        return None, status
    if not curve:
        return None, status
    last_epoch = max(curve.keys())
    return curve[last_epoch], "fallback_last_epoch"
...
def parse_str_set(csv_text: str):
    return {x.strip() for x in csv_text.split(",") if x.strip()}


def normalize_num_token(raw: str) -> str:
    s = f"{float(raw):.6g}"
    return s.rstrip("0").rstrip(".") if "." in s else s


def parse_num_token_set(csv_text: str):
    return {
        normalize_num_token(raw.strip())
        for raw in csv_text.split(",")
        if raw.strip()
    }


def make_output_name(info: dict, epochs_tag: str, client_tag: str):
    base = (
        f"num_clients_sensitivity_{info['iid']}_{info['dataset']}_{info['model']}_{info['algo']}"
        f"_alpha{info['alpha']}_adv{info['adv']}_lr{info['lr']}"
        f"_cfg{info['cfg']}_clientset{client_tag}_epochs{sanitize(epochs_tag.replace(',', '-'))}"
    )
...
def main():
    ...
    client_filter = parse_int_list(args.client_values) if args.client_values.strip() else []
    dataset_filter = parse_str_set(args.dataset_filter) if args.dataset_filter.strip() else set()
    if args.adv_values.strip():
        adv_filter = parse_num_token_set(args.adv_values)
    else:
        adv_filter = set()
    ...
    filtered_by_iid_mode = 0
    filtered_by_dataset = 0
    filtered_by_clientset = 0
    filtered_by_adv = 0
    curve_cache = {}
    ...
    for path in all_txt:
        ...
        if meta is None:
            ...
            continue
        if args.iid_mode != "all" and meta["iid"] != args.iid_mode:
            filtered_by_iid_mode += 1
            continue
        if dataset_filter and meta["dataset"] not in dataset_filter:
            filtered_by_dataset += 1
            continue
        if adv_filter and normalize_num_token(meta["adv"]) not in adv_filter:
            filtered_by_adv += 1
            continue
        if client_filter and int(meta["num_clients"]) not in client_filter:
            filtered_by_clientset += 1
            continue
    ...
    generated = 0
    skipped_no_points = 0
    skipped_single_client_value = 0
    discarded_short_runs = 0
    discarded_missing_target_runs = 0
    fallback_last_epoch_single_seed42_runs = 0

    for gk, records in grouped.items():
        info = key_to_dict(gk)

        client_values = sorted({int(r["num_clients"]) for r in records})
        if client_filter:
            client_values = [c for c in client_filter if c in set(client_values)]
        if len(client_values) < 2:
            skipped_single_client_value += 1
            continue

        by_attack_def_client = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        baseline_by_client = defaultdict(list)
        epochs_seen = set()
        for rec in records:
            epochs_seen.add(rec["epochs"])
            n_clients = int(rec["num_clients"])
            if rec["attack"] == BASELINE_ATTACK and rec["defense"] == BASELINE_DEFENSE:
                baseline_by_client[n_clients].append(rec)
            if rec["attack"] in attacks:
                by_attack_def_client[rec["attack"]][rec["defense"]][n_clients].append(rec)
        ...
        for ax, attack in zip(axes, attacks):
            ...
            ax.set_xlabel("Number of Clients", fontsize=BASE_FONT_SIZE * FONT_SCALE)
            ax.set_ylabel("Test Accuracy", fontsize=BASE_FONT_SIZE * FONT_SCALE)
            ax.grid(alpha=0.2)
            ax.tick_params(labelsize=BASE_FONT_SIZE * FONT_SCALE)

            plotted_on_ax = False
            defense_to_client_stats = defaultdict(dict)
            for defense, client_map in by_attack_def_client.get(attack, {}).items():
                if defense in EXCLUDED_ATTACK_DEFENSES:
                    continue
                for n_clients, recs in client_map.items():
                    by_seed = defaultdict(list)
                    for rec in recs:
                        by_seed[rec["seed"]].append(rec)

                    vals = []
                    seed_keys = sorted(by_seed.keys(), key=lambda x: int(x) if x.isdigit() else x)
                    for seed, same_seed_recs in by_seed.items():
                        ...
                        if status == "short":
                            acc, status = maybe_use_last_epoch_for_single_seed42(
                                curve, seed, seed_keys, status
                            )
                            if status == "fallback_last_epoch":
                                fallback_last_epoch_single_seed42_runs += 1
                            else:
                        ...
                        seeds_used.add(seed)
                    if vals:
                        defense_to_client_stats[defense][n_clients] = {
            ...
            for defense in sorted(defense_to_client_stats.keys(), key=defense_sort_key):
                points = sorted(
                    [
                        (n_clients, defense_to_client_stats[defense][n_clients])
                        for n_clients in client_values
                        if n_clients in defense_to_client_stats[defense]
                    ],
                    key=lambda kv: kv[0],
                )
                if not points:
            ...
            baseline_points = []
            for n_clients in client_values:
                recs = baseline_by_client.get(n_clients, [])
                by_seed = defaultdict(list)
                for rec in recs:
                    by_seed[rec["seed"]].append(rec)
                vals = []
                ...
                if vals:
                    baseline_points.append(
                        (n_clients, float(np.mean(vals)), float(np.std(vals)), len(vals))
                    )
                    curve_ns.append(len(vals))
                    plotted_on_ax = True
                    plotted_any = True
            ...
            ax.set_xticks(client_values)
            ax.set_xticklabels([str(x) for x in client_values])
        ...
        title = (
            f"num_clients sensitivity | {info['iid']} | {info['dataset']}_{info['model']} | {info['algo']} | "
            f"adv={info['adv']} | alpha={info['alpha']} | "
            f"epochs_src={epochs_tag} | cfg={info['cfg']} | mean±std over available seeds"
        )
        if seeds_sorted:
        ...
        if legend_handles:
            legend_order = [
                ...
                if d in legend_handles and d not in EXCLUDED_ATTACK_DEFENSES
            ]
            for d in sorted(legend_handles.keys(), key=defense_sort_key):
                if d not in legend_order and d != BASELINE_LABEL:
                    legend_order.append(d)
            if BASELINE_LABEL in legend_handles:
                legend_order.append(BASELINE_LABEL)
            fig.legend(
                [legend_handles[d] for d in legend_order],
                legend_order,
                loc="lower center",
        ...
        client_tag = "-".join(str(x) for x in client_values)
        out_name = make_output_name(info, epochs_tag, client_tag)
        ...
        rows.append(
            {
                ...
                **info,
                ...
                "client_values": ",".join(str(x) for x in client_values),
    ...
    index_csv = args.out_dir / "figure_index.csv"
    ...
    summary_lines = [
        ...
        f"parsed_after_filters={len(parsed)}",
        f"filtered_by_iid_mode={filtered_by_iid_mode}",
        f"filtered_by_dataset={filtered_by_dataset}",
        f"filtered_by_adv={filtered_by_adv}",
        f"filtered_by_clientset={filtered_by_clientset}",
        ...
        f"discarded_short_runs={discarded_short_runs}",
        f"discarded_missing_target_runs={discarded_missing_target_runs}",
        f"fallback_last_epoch_single_seed42_runs={fallback_last_epoch_single_seed42_runs}",
        f"groups_total={len(grouped)}",
        f"generated_figures={generated}",
        f"skipped_no_points={skipped_no_points}",
        f"skipped_single_client_value={skipped_single_client_value}",
        f"target_epoch={args.target_epoch}",
        f"client_filter={','.join(str(x) for x in client_filter) if client_filter else 'ALL'}",
        f"iid_mode={args.iid_mode}",
        f"dataset_filter={','.join(sorted(dataset_filter)) if dataset_filter else 'ALL'}",
        f"adv_filter={','.join(sorted(adv_filter)) if adv_filter else 'ALL'}",
        ...
        f"figure_index={index_csv}",
    ]
    summary_path = args.out_dir / "summary.txt"
    summary_path.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")

    if summary_rows:
        coverage_csv = args.out_dir / "coverage_by_group.csv"
