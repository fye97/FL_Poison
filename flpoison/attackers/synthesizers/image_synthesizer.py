...
from flpoison.utils.config_utils import repo_root
...
class ImageSynthesizer(Synthesizer):
    ...
    def _resolve_trigger_path(self) -> Path:
        raw = Path(self.trigger_path).expanduser()
        candidates = []
        ...
        if raw.is_absolute():
            ...
            if raw.parts and raw.parts[0] != "flpoison":
                candidates.append(repo_root() / "flpoison" / raw)
        ...
        raise FileNotFoundError(f"trigger image not found: {self.trigger_path}")
    ...
    def setup_trigger(self, trigger=None):
        ...
        trigger_img = Image.open(self._resolve_trigger_path()).convert(mode)
